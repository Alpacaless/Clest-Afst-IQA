...
class PMIQDFolder(data.Dataset):
    def __init__(self, root, index, transform, patch_num):
        """
        适配 mos.xlsx 无表头、A列存分数的场景
        :param root: 数据集根目录（含 IMGS + mos.xlsx）
        :param index: 选中的图片索引（对应 IMGS 排序后下标）
        :param transform: 图像预处理
        :param patch_num: 每张图生成的 patch 数
        """
        # 1. 读取 mos.xlsx（无表头，直接取 A 列）
        mos_path = os.path.join(root, 'mos.xlsx')
        self.mos_scores = pd.read_excel(mos_path, header=None, usecols=[0]).values.flatten().astype(np.float32)
        # 检查分数数量是否匹配图片数量
        img_dir = os.path.join(root, 'IMGS')
        self.all_img_names = sorted([f for f in os.listdir(img_dir) if f.endswith(('.jpg', '.jpeg', '.png'))])
        # assert len(self.mos_scores) == len(self.all_img_names), "分数数量和图片数量不匹配！"

        # 2. 归一化分数
        min_score = self.mos_scores.min()
        max_score = self.mos_scores.max()
        self.norm_scores = (self.mos_scores - min_score) / (max_score - min_score)

        # 3. 构建样本列表（路径 + 归一化分数）
        self.img_paths = [os.path.join(img_dir, name) for name in self.all_img_names]
        self.samples = []
        for idx in index:
            score = self.norm_scores[idx]
            for _ in range(patch_num):
                self.samples.append((self.img_paths[idx], score))
